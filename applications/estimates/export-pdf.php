...
switch(Core::$request->method) {
	...
	case 'estimateExpPdf':
		...
		if ($id_estimate > 0) {
			...
			$datapdf = array();
			$datapdf[0]['col1'] = '<strong>'.strtoupper($_lang['voce']).'</strong> '.$_lang['del'].' <strong>'.DateFormat::convertDateFormats($App->item->dateins,'Y-m-d',$_lang['data format'],$App->dateNow).'</strong>';
			$datapdf[0]['col2'] = '<strong>'.strtoupper($_lang['scadenza']).'</strong>: '.DateFormat::convertDateFormats($App->item->datesca,'Y-m-d',$_lang['data format'],$App->dateNow);
			$opt = array(
				'showHeadings'=>0,
				'gridlines'=>EZ_GRIDLINE_DEFAULT,
				'fontSize'=>9,
