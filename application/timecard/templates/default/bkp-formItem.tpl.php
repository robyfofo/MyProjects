...
<div class="row">
	<div class="col-md-4">
		...
		<table class="table table-striped table-bordered table-hover timecards">
			<?php if (is_array($this->App->dates_month) && count($this->App->dates_month) > 0): ?>
				...
				<?php foreach ($this->App->dates_month AS $day): ?>
					<tr>
						...
						<td>
							<?php 
								$timecards = $this->App->timecards[$day['value']]['timecards'];								
							?>
							
							<?php if (is_array($timecards) && count($timecards) > 0): ?>
								<table class="table table-striped table-bordered table-hover subtimecards">
									<tbody>
										<?php foreach ($timecards AS $value): ?>
											<tr>
	...
	<div class="col-md-8">
		...
		<form id="applicationForm" class="form-inline" role="form" action="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>"  enctype="multipart/form-data" method="post">
			<fieldset>
