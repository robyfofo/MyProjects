<!-- admin/site-files/listItem.tpl.php v.3.0.0. 02/11/2016 -->
<div class="row">
					...
					<div class="col-md-12">
						<div class="form-group">
							...
							<select class="form-control input-md" name="id_folder" onchange="this.form.submit();" >
								<option value="0"<?php if($this->App->id_folder == 0) echo ' selected="selected"'; ?>>Tutte</option>
								<?php if (is_array($this->App->foldersData) && count($this->App->foldersData) > 0): ?>
									<?php foreach($this->App->foldersData AS $key=>$value): ?>												
										<option value="<?php echo $value->id; ?>"<?php if(isset($this->App->id_folder) && $this->App->id_folder == $value->id) echo ' selected="selected"'; ?>><?php echo htmlspecialchars($value->title_it,ENT_QUOTES,'UTF-8'); ?></option>														
									<?php endforeach; ?>
								<?php endif; ?>	
							</select>									
						</div>
								...
								?>
									<tr>
										<?php if (isset($this->App->userLoggedData->is_root) && $this->App->userLoggedData->is_root === 1): ?>
											...
											<td><?php echo $value->id_folder; ?></td>
										<?php endif; ?>										
										<td><?php echo SanitizeStrings::htmlout($value->title_it); ?></td>
										<td>	
											<a class="" href="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/downloadItem/<?php echo $value->id; ?>" title="Scarica il file">
												<?php echo $value->filename; ?>
											</a>	
										</td>	
										<td>												
											<button type="button" class="popoverInfo btn btn-default btn-circle" data-container="body" data-toggle="popover" data-placement="top" data-content="<?php echo $this->App->params->uploadDirs['item']; ?><?php echo $value->folder_name; ?><?php echo $value->filename; ?>">
