 ...
 * admin/site-images/folders.php v.3.0.0. 02/11/2016
*/
			   		...
			   		if (Core::$resultOp->error == 0) {   	
			   			...
			   			if (Core::$resultOp->error == 0) {		   			
				   			...
				   			if ($oldFold_name != $newFold_name) {
				   				...
				   				Sql::updateRecord();
				   				if (Core::$resultOp->error == 0) {
				   					Core::$resultOp->messages[] = 'Ho cambiato il folder name alle '.$App->params->labels['fold']['sons'].' associat'.$App->params->labels['fold']['sonsSex'].'!';	
										}				   					
				   				}   			
								}	
			   			}
...
switch((string)$App->viewMethod) {
	...
	case 'list':
		...
		$qryFields = array('c.*','(SELECT COUNT(i.id) FROM '.$App->params->tables['item'].' AS i WHERE i.id_folder = c.id) AS numimages');
