<!-- admin/site-images/listImages.tpl.php v.3.0.0. 02/11/2016 -->
<div class="row">
							...
							<select class="form-control input-md" name="id_folder" onchange="this.form.submit();" >
								...
								<?php if (is_array($this->App->foldersData) && count($this->App->foldersData) > 0): ?>
									<?php foreach($this->App->foldersData AS $key=>$value): ?>												
										<option value="<?php echo $value->id; ?>"<?php if(isset($this->App->id_folder) && $this->App->id_folder == $value->id) echo ' selected="selected"'; ?>><?php echo SanitizeStrings::cleanForFormInput($value->title_it); ?></option>														
									<?php endforeach; ?>
								<?php endif; ?>	
							</select>									
						</div>					
